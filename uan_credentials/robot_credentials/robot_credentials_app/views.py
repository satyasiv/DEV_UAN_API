...
class run_robot(APIView):
    def post(self, request, *args, **kwargs):
        ...
        # Generate UUID based on user credentials
        ...
        # Mock response for testing (replace with your actual API call)
        response = requests.get('https://app.hoppr.in/api/uan-generation-linking-process?username=adminapi&key=eshyosdmtoopcfrgxthftnfwthdicxmmioxedozotygbztmzfecjmxznzersteiketxusgydkpcqcljeptabzxthzmuvpyrfvjgdcwlytxpdvcfwmhytzvkzvrqammra&data_count=5')
        data = response.json()   
        validated_data = []
        for instance in data:
            ...
            validated_data.append({
                ...
                'date_of_birth': self.convert_to_dd_mm_yyyy(instance['date_of_birth']),
                'date_of_joining': self.convert_to_dd_mm_yyyy(instance['date_of_joining']),
                'Universal_Account': universal_account,
                'wages_as_on_joining': instance['wages_as_on_joining'],
            })
        
        threading.Thread(target=self.run_bot, args=(validated_data,)).start()
        return Response({
            'message': 'Robot is in progress for valid users. Please view the status from localhost:8000/boturl to know about the bot status',
            'uuid': str(user_uuid),
            'timestamp': timestamp
        }, status=status.HTTP_200_OK)
    
    # Function to convert date to dd/mm/yyyy format
    def convert_to_dd_mm_yyyy(self, date_input):
    ...
    # Function to run the bot process
    def run_bot(self, validated_data):
        robot_path = "/home/buzzadmin/Documents/uan_credentials/robot_credentials/robot_credentials_app/tasks.robot"          
